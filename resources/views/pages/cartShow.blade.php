...
<div class="max-w-4xl mx-auto px-4 py-8 md:py-12 min-h-screen">
    ...
    {{-- ✅ Checkout All Button --}}
    <div class="mt-4">
        <form method="POST" action="{{ route('cart.checkoutAll') }}">
            @csrf
            <button type="submit"
                class="w-full py-3.5 px-4 bg-gray-900 hover:bg-gray-800 text-white font-semibold rounded-xl 
                           transition-all duration-200 hover:shadow-lg active:scale-[0.98] flex items-center justify-center gap-2">
                <i class="fa-solid fa-bag-shopping"></i>
                Checkout All Items ({{ $cartItems->count() }})
            </button>
        </form>
    </div>
    @endif
